...
if __name__ == "__main__":
    ...
    sequence_folders = [
        f
        for f in sorted(os.listdir(sequences_dir))
    ...
    for folder in sequence_folders:
        ...
        if (
            ...
            or os.path.exists(velodyne_folder)
            or os.path.exists(labels_folder)
        ):
        ...
        scan_files = [
            f
            for f in sorted(os.listdir(os.path.join(input_folder, "velodyne")))
            if f.endswith(".bin")
        ]
        ...
        for i, f in enumerate(scan_files):
            ...
            label_filename = os.path.join(
                input_folder, "labels", os.path.splitext(f)[0] + ".label"
            )
            ...
            start = 0
            concated_points[4 * start : 4 * (start + points.shape[0])] = scan.reshape(
                (-1)
            )
            ...
            for past in history:
                ...
                concated_points[
                    4 * start : 4 * (start + past["points"].shape[0])
                ] = tpoints
                concated_labels[start : start + past["labels"].shape[0]] = past[
                    "labels"
                ]
            ...
            concated_labels.tofile(
                os.path.join(labels_folder, os.path.splitext(f)[0] + ".label")
            )
