...
if __name__ == "__main__":
    ...
    for seq in test_sequences:
        ...
        seq_dir_pred = os.path.join(
            "sequences", "{0:02d}".format(int(seq)), "predictions"
        )

        gt_file_list = [
            f
            for f in os.listdir(os.path.join(args.dataset, seq_dir_gt))
            if f.endswith(".label")
        ]
    ...
    if missing_pred_files:
        raise RuntimeError("Error: Missing prediction files! Aborting evaluation.")
    ...
    print(
        "Precision =\t"
        + str(np.round(precision * 100, 2))
        + "\n"
        + "Recall =\t"
        + str(np.round(recall * 100, 2))
        + "\n"
        + "IoU Cmpltn =\t"
        + str(np.round(acc_cmpltn * 100, 2))
        + "\n"
        + "mIoU SSC =\t"
        + str(np.round(mIoU_ssc * 100, 2))
    )
